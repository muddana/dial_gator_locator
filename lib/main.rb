...
class Route
 ...
 class << self; attr_accessor :base_url, :route_number_to_name, :rts_to_transloc_mapper, :abbreviations end;
  ...
  @rts_to_transloc_mapper =  {16=>66, 5=>75, 17=>67, 12=>61, 34=>59, 300 => 3, 301=>4, 13=>65, 35=>68, 302=>1, 8=>63, 20=>55, 9=>64, 26=>40, 10=>76}.merge({126=>40, 401=>71, 407=>48, 402=>57, 408=>74, 403=>72, 405=>78, 404=>78,400=>58, 406=>56})
  ...
  def initialize(route_num)
    ...
    @route_name = Route.route_number_to_name[route_num]
    @transloc_id = Route.rts_to_transloc_mapper[route_num]
  end
  ...
  def get_info
    @busses = Array.new
    doc = open( "#{Route.base_url}/route.php?id=#{@transloc_id}") { |f| Hpricot(f) }
    doc.search('li')[0..-3].each do |bus_html|
      ...
      Route.abbreviations.each_pair { |key, val| bus_info.gsub!(key, val) }
      @busses << bus_info
    end
    [@route_name, @busses]
  end
